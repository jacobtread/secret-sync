...
/// Tests quick pulling a configuration file from a AWS secret manager
#[tokio::test]
async fn test_quick_pull_aws() {
...
/// Tests quick pulling a unknown configuration file from AWS secret manager
#[tokio::test]
async fn test_quick_pull_aws_unknown() {
    let temp_test_file = NamedTempFile::new().unwrap();
    let temp_test_file_path = temp_test_file.path();
    let temp_test_file_path_display = normalize_test_path(temp_test_file_path);

    let (_secret_manager, config_temp_file, _container) = test_harness_aws(toml::Table::default()).await;
